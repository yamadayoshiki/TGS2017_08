...
bool Collision::Box_Box(const std::vector<GSvector2> box1, const std::vector<GSvector2> box2, HitInfo & hitinfo){
	// 線分
	GSvector2 segment1[2] = {};
	GSvector2 segment2[2] = {};
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			...
			segment2[1] = box2[j + 1];
			// 交差の判定
			if (Collision::Segment_Segment(segment1, segment2, intersect)) {
				// 最初の要素へのイテレータを返す
	...
	// 配列のサイズが0(交点数が0)でない場合交差している
	return hitinfo.intersects.size() > 0;
}

bool Collision::Segment_Box(const std::vector<GSvector2> segment, const std::vector<GSvector2> box, HitInfo & hitinfo){
	// 線分
	GSvector2 segment1[2] = { segment[0], segment[1] };
	GSvector2 segment2[2] = {};
