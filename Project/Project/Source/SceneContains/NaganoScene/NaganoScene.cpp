...
void NaganoScene::OnStart()
{
	...
	p_World->addActor(ActorGroup::Player, std::make_shared<TestObj2>(p_World.get(), GSvector2(50, 100)));
	p_World->addActor(ActorGroup::Enemy, std::make_shared<TestEnemy>(p_World.get(), GSvector2(100, 100)));
}
