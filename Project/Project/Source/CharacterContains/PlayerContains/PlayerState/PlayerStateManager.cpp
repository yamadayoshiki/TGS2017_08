...
PlayerStateManager::PlayerStateManager(GSvector2 & position, GSmatrix4& matrix, IGameManagerPtr gameManager)
{
	...
	//通常状態
	add(PlayerStateName::Close,	std::make_shared<PlayerState_Close>(position, matrix, gameManager));
	...
	//潰す状態
	add(PlayerStateName::Crush,			std::make_shared<PlayerState_Crush>			(position,matrix, gameManager));
