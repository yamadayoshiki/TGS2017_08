#include "CsvConvertTwoDVector.h"
#include <GSvector2.h>
#include <algorithm>
...
TwoDVector CsvConvertTwoDVector::GetTwoDVector(const std::string & fileName, int interval) {
	//結果変数
	TwoDVector result;
...
GSvector2 CsvConvertTwoDVector::CsvPosCnvVector2(const int x, const int y, const MapType type) {
	//結果変数
	GSvector2 result;
	//1マスの長さ
...
std::vector<CsvCellData> CsvConvertTwoDVector::GetPatrolPos(const std::string & fileName) {
	//結果変数
...
std::vector<GSvector2> CsvConvertTwoDVector::GetPatrolPosList(const std::vector<CsvCellData> dataList, MapType type) {
	//結果変数
