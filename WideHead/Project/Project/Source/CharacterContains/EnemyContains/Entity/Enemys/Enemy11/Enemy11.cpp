...
ActorPtr Enemy11::CsvGenerate(const int x, const int y, const int csvparam)
{
	...
	return std::make_shared<Enemy11>(p_World, position, dir, p_GameManager);
}

void Enemy11::SetUpCommand() {
	...
	m_TextureMap["RushPreliminary"] = std::make_shared<AnimationTexture>("Enemy09RushPreliminary", p_GameManager->GetDrawManager(), DrawOrder::Enemy, 32, 8);
...
void Enemy11::SetUpState() {
	...
	p_StateManager->add(EnemyStateName::RushPreliminary, std::make_shared<EnemyStateRushPreliminary>(shared_from_this(), 0));
